...
CFG_DIR = Path(os.environ.get("LOCALAPPDATA", str(Path.home()))) / "fuckdpi"
...
HELP_LINES = [
    ...
    ("/fuckdpi select",   "FuckDPI — только список доменов через winws"),
    ("/fuckdpi all",      "FuckDPI — весь трафик через winws"),
    ("/help",             "эта справка"),
...
def elevate_and_rerun():
    """Перезапуск с правами администратора (UAC)."""
    args = " ".join(f'"{a}"' for a in sys.argv)
    ctypes.windll.shell32.ShellExecuteW(
        None, "runas", sys.executable, f'"{__file__}" {args[1:]}',
        None, 1)
    sys.exit(0)


def sb_bin() -> str:
    local = CFG_DIR / "sing-box.exe"
    if local.exists():
        return str(local)
    for p in ["sing-box.exe", r"C:\Program Files\sing-box\sing-box.exe"]:
        if os.path.exists(p):
            return p
    return "sing-box.exe"
...
def fetch_subscription(url: str, log=lambda m: None):
    attempts = [
        ["curl.exe", "-sL", "--max-time", "25", "-A", UA_BROWSER, url],
        ["powershell", "-Command",
         f"(Invoke-WebRequest -Uri '{url}' -UserAgent "
         f"'{UA_BROWSER}' -TimeoutSec 25).Content"],
    ]
    for i, cmd in enumerate(attempts, 1):
        tool = cmd[0]
        log(f"загрузка подписки ({tool}, попытка {i}/2)...")
...
def exit_ip(timeout=6):
    for _attempt in range(2):
        for url in ("https://api.ipify.org", "https://ifconfig.me",
            ...
            r = run(["curl.exe", "-4", "-s", "--max-time", str(timeout), url],
                    timeout=timeout + 3)
...
def service_control(action: str) -> int:
    """sc.exe start/stop/restart fuckdpi."""
    if action == "start":
        r = run(["sc", "start", SERVICE_NAME], timeout=15)
        return r.returncode
    elif action == "stop":
        r = run(["sc", "stop", SERVICE_NAME], timeout=15)
        return r.returncode
    elif action == "restart":
        run(["sc", "stop", SERVICE_NAME], timeout=10)
        time.sleep(2)
        r = run(["sc", "start", SERVICE_NAME], timeout=15)
        return r.returncode
    return 1
...
def service_install():
    """Установить sing-box как Windows-сервис."""
    sb = sb_bin()
    cfg = str(CONFIG_FILE)
...
def service_uninstall():
    run(["sc", "stop", SERVICE_NAME], timeout=10)
    run(["sc", "delete", SERVICE_NAME], timeout=10)


def fuckdpi_running() -> bool:
    r = run(["tasklist", "/FI", "IMAGENAME eq winws.exe", "/NH"],
            timeout=5)
    return "winws.exe" in (r.stdout or "").lower()
...
def fuckdpi_start(mode: str, log=lambda m: None) -> bool:
    ...
    if mode == "select" and not hostlist:
        log("список доменов пуст; добавь через /list")
        return False
    ...
    if hostlist:
        cmd.append(f"--hostlist={HOSTLIST_FILE}")

    log(f"запуск FuckDPI (winws, {mode})...")
    try:
        proc = subprocess.Popen(cmd,
                                creationflags=subprocess.CREATE_NO_WINDOW)
        time.sleep(1)
        if proc.poll() is not None:
            log(f"winws завершился с кодом {proc.returncode}")
            return False
        return True
    except Exception as e:
        log(f"ошибка запуска winws: {e}")
        return False


def fuckdpi_stop(log=lambda m: None):
    run(["taskkill", "/IM", "winws.exe", "/F"], timeout=5)


# ---------------------------------------------------------------- конфиг sing-box (Windows)
...
class Engine:
    ...
    def start(self, bypass="all"):
        ...
        if ok and ip:
            self.say(f"VPN подключен ({label}) . IP: {ip}")
        elif ok:
            self.say(f"туннель поднят ({label}); IP определяется...")
        else:
            self.say(f"интерфейс не появился за 15 с — /log")

    def stop(self):
        if service_active():
            rc = service_control("stop")
        else:
            rc = 0
        if fuckdpi_running():
        ...
        if rc == 0 and gone_tun and gone_nf:
            ...
            self.say(f"остановлено (sc={rc}, tun={gone_tun}, "
                     f"winws={gone_nf})")
    ...
    def logs(self, n=30):
        r = run(["powershell", "-Command",
                 f"Get-EventLog -LogName Application -Source '{SERVICE_NAME}' "
                 f"-Newest {n} | Select-Object -ExpandProperty Message"],
                timeout=10)
        if r.returncode == 0 and r.stdout.strip():
            return r.stdout.rstrip().splitlines()[-n:]
        return ["логи недоступны; попробуй:",
                f"  Get-EventLog -LogName Application -Source '{SERVICE_NAME}'"]
...
class UI:
    ...
    def _editor_draw(self):
        ...
        hdr = "  FuckDPI Windows -- редактор списка доменов"
    ...
    def loop(self):
        self.post("FuckDPI Windows ready. /help -- commands.")
        while True:
...
def cli_main(args):
    ...
    if cmd in ("help", "--help", "-h"):
        print("fuckdpi-windows -- управление VPN + FuckDPI\n")
        for c, d in HELP_LINES:
            print(f"  {c:<20} {d}")
